...
const getOne = catchError(async(req, res) => {
    ...
    if(!result) return res.status(404).json({message: "Director not found"})
...
const remove = catchError(async(req, res) => {
    ...
    if (result === 0) return res.status(404).json({message: "Director not found"})
...
const update = catchError(async(req, res) => {
    ...
    if(result[0] === 0) return res.status(404).json({message: "Director not found"})
